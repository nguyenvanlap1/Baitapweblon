...
exports.findAll = async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new ApiError(
        500,
        `An error occurred while retrieving employees: ${error}`
      )
    );
  }
};
...
exports.findOne = async (req, res, next) => {
  try {
    ...
    return next(
      new ApiError(
        500,
        `Error retrieving employee with id=${req.params.id}: ${error}`
      )
    );
  }
};
...
exports.update = async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new ApiError(
        500,
        `Error updating employee with id=${req.params.id}: ${error}`
      )
    );
  }
};
...
exports.deleteAll = async (req, res, next) => {
  try {
    ...
    return next(
      new ApiError(
        500,
        `An error occurred while removing all employees: ${error}`
      )
    );
  }
};
