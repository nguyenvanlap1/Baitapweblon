...
exports.login = async (req, res, next) => {
  try {
    ...
    req.session.user = {
      _id: reader[0]._id,
      diachi: reader[0].diachi,
      dienthoai: reader[0].dienthoai,
      holot: reader[0].holot,
      ngaysinh: reader[0].ngaysinh,
      phai: reader[0].phai,
      ten: reader[0].ten,
    };
...
exports.getProfile = async (req, res, next) => {
  try {
    ...
    // Nếu là nhân viên
    if (user.chucvu) {
      const employee = await nhanVienService.findById(user._id);
      if (!employee || employee.length === 0) {
    ...
    // Nếu là độc giả
    const reader = await docGiaService.findById(user._id);
    if (!reader || reader.length === 0) {
      return next(new ApiError(404, "Reader not found"));
    }
    return res.send(reader[0]);
  } catch (error) {
    return next(new ApiError(500, error.message));
  }
};
